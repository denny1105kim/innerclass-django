...
# ✅ analyze_news는 "1회 호출 -> Lv1~Lv5 분배 저장 + theme 저장"을 내부에서 수행하는 것으로 전제
...
class Command(BaseCommand):
    ...
    # ✅ market 저장 값(프로젝트에 맞게 유지)
    # - 당신의 기존 코드가 "International" 문자열을 쓰고 있으므로 그대로 둠
        ...
        try:
            with transaction.atomic():
                article = NewsArticle.objects.create(
                    ...
                    # ✅ 아래 2개는 당신이 기존에 쓰던 필드가 있으면 유지/수정하세요
                    # sector는 기존 코드에서 문자열로 저장하고 있었음
                    sector="금융/경제",
                    ticker=None,
                ...
                # ✅ 여기서 1회 호출로 Lv1~Lv5 row 저장 + 대표 theme(NewsArticle.theme) 저장까지 끝내는 구조
